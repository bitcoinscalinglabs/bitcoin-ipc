...
def plot3(filename, title, n_transfers_list, x_ticks):
    ...
    for n_subnets in n_target_subnets_list:
        ...
        if pairs:
            x, y = zip(*pairs)
            plt.plot(x, y, marker='o', label=f'{n_subnets} target subnets')
...
for n_transfers in n_transfers_list_default:
    plt.plot(validators_dict[n_transfers], total_sizes_dict[n_transfers], marker='o', label=f'{n_transfers} total transfers')
...
for n_transfers in n_transfers_list_for_plot:
    ...
    plt.plot(validators_dict[n_transfers], avg_sizes, marker='o', label=f'{n_transfers} total transfers')
...
# Third plot
plot3('bench-plots/3-amortized_vsize_vs_n_transfers_vary_n_subnets.png', 'Amortized size per transfer vs. total number of transfers (4 validators)', n_transfers_list_default, [5,10,15,20,25,30,35,40,45,50])

# Third plot, take it to limits
plot3('bench-plots/3B-amortized_vsize_vs_n_transfers_vary_n_subnets.png', 'Amortized size per transfer vs. total number of transfers (4 validators)',[1,2,3,5, 10,20,50, 100, 200, 500, 1000, 10000, 15000], [0,1000, 2000, 5000, 10000, 15000])

# Third plot, zoom in
plot3('bench-plots/3C-amortized_vsize_vs_n_transfers_vary_n_subnets.png', 'Amortized size per transfer vs. total number of transfers (4 validators, zoom-in)', [1,2,3,5, 10], [1,2,3,5, 10])
...
for n_validators in n_validators_list:
    ...
    if pairs:
        x, y = zip(*pairs)
        plt.plot(x, y, marker='o', label=f'{n_validators} validators')
