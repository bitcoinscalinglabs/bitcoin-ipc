...
run_validator() {
    ...
    for svc in fendermint cometbft ethapi; do
        if ! docker ps --format '{{.Names}}' | grep -q "^validator-${validator_num}-${svc}$"; then
            echo "Error: validator-${validator_num}-${svc} container not running after cargo-make completed." >&2
            return 1
        fi
    done
...
for args in \
    "2 26756 26757 8645 26755 $API_URL_2 $BEARER_TOKEN_2" \
    "3 26856 26857 8745 26855 $API_URL_3 $BEARER_TOKEN_3" \
    "4 26956 26957 8845 26955 $API_URL_4 $BEARER_TOKEN_4"
do
    # shellcheck disable=SC2086
    set -- $args
    validator_num=$1
    if ! run_validator "$@"; then
        echo "Error: validator $validator_num failed to start. See log: /root/.ipc/logs/spin-up-subnet-a-validator-${validator_num}.log" >&2
        echo "Aborting." >&2
        exit 1
    fi
done
